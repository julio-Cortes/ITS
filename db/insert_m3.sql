...
INSERT INTO questions (id, question, hint, type, query, difficulty, module_id) VALUES (31, 'Se tiene el siguiente modelo:
 ORDERS(OrderID, CustomerID, OrderDate)
 Customers(CustomerID,CustomerName, ContactName, Address, City,PostalCode,Country) 
Haga una consulta que seleccione todos los CustomerName junto con su City.', 'Recuerde que todo lo que se pide en la consulta debe estar en el SELECT',1, 'SELECT CustomerName,City FROM Customers;',1,3);


INSERT INTO questions (id, question, hint, type, query, difficulty, module_id) VALUES (32, '
Se tiene el siguiente modelo:
 ORDERS(OrderID, CustomerID, OrderDate)
 Customers(CustomerID,CustomerName, ContactName, Address, City,PostalCode,Country) 
Haga una consulta que seleccione solo los valores distintos de la columna "Country" en la tabla "Customers".', 'Recuerde que para seleccionar los distintos se utiliza DISTINCT',1, 'SELECT DISTINCT Country FROM Customers;',1,3);


INSERT INTO questions (id, question, hint, type, query, difficulty, module_id) VALUES (33, 'Se tiene el siguiente modelo:
 ORDERS(OrderID, CustomerID, OrderDate)
 Customers(CustomerID,CustomerName, ContactName, Address, City,PostalCode,Country) 
Haga una consulta que seleccione los Customers que viven en "Germany" o en "Spain" y se llaman "Pepe".', 'Recuerde utilizar combinaciones de OR y AND para las condiciones',1, 'SELECT * FROM Customers WHERE (Country="Germany" OR Country="Spain") AND CustomerName = "Pepe";',2,3);


INSERT INTO questions (id, question, hint, type, query, difficulty, module_id) VALUES (34, 'Se tiene el siguiente modelo: 
PAIS(*nombre*, impuesto, costo_envio) 
PRODUCTO(*id*, nombre, precio, origen) donde origen es clave foranea de Pais 
Para cada producto mostrar nombre, precio, costo de envío y impuesto correspondiente al país', 'Recuerde mostrar todo lo que se pide en el SELECT',1, 'SELECT pr.nombre , pr.precio, pa.costo_envio, pa.impuesto From Pais pa, Producto pr WHERE pr.origen = pa.nombre;',3,3);


INSERT INTO questions (id, question, hint, type, query, difficulty, module_id) VALUES (35, 'Se tiene el siguiente modelo: 
PAIS(*nombre*, impuesto, costo_envio) 
PRODUCTO(*id*, nombre, precio, origen) donde origen es clave foranea de Pais 
...
Para la venta  de id 3, mostrar Nombre, Precio y Cantidad de los productos que fueron vendidos', 'Recuerde utilizar las condiciones necesarias con AND y WHERE',1, 'SELECT pv.roducto, v.recio, pv.cantidad FROM ProductoVenta pv, Venta v WHERE v.ID = 3 AND pv.venta=3;',3,3);


INSERT INTO questions (id, question, hint, type, query, difficulty, module_id) VALUES (36, 'Se tiene el siguiente modelo:
 ORDERS(OrderID, CustomerID, OrderDate)
 Customers(CustomerID,CustomerName, ContactName, Address, City,PostalCode,Country) 
Haga una consulta que seleccione los Customers que no viven en "Spain".', 'Recuerde utilizar NOT para los negados',1, 'SELECT * FROM Customers WHERE NOT Country="Spain";',1,3);